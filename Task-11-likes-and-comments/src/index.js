...
function router() {
    ...
    if (DataService.getCurrLoggedInUserFromLocalStorage() !== null) {
        document.getElementById("userProfile").setAttribute("src", `${(DataService.getCurrLoggedInUserFromLocalStorage()).avatar}`);
        document.getElementById("userProfile").setAttribute("alt", "Avatar");
    }
    ...
    if( url === '#' ) {
        ...
        routes[url]
            .render()
            .then(result => root.append(result));
...
document.addEventListener('click', async function(e) {
    ...
    const likeBtnIdRegex = /likeBtn_(.*)_post_(.*)/g;
    const commentBtnIdRegex = /commentBtn_(.*)/g;
    const replyBtnIdRegex = /replyBtn_(.*)/g;
    const showCommentsRegex = /showComments_(.*)_post_(.*)/g;
    const showRepliesRegex = /showReplies_(.*)_comment_(.*)/g;
